...
def build():
    ...
    text = (
        "При одинаковых V, v0 и τ = 2 мин во всех трёх случаях РИВ лучше РИС-н: выше степень "
        "превращения X, селективность SR и производительность ΠR по целевому продукту R. "
        "В РИС-н весь объём работает при низкой выходной CA, поэтому стадия A → R идёт медленнее.\n\n"
        "Случай 1 (n1 = 2) наименее выгоден — целевая реакция тормозится по мере расхода A. "
        "Случаи 2 и 3 дают одинаковое X (n1 = 1), но случай 3 (n2 = 2) лучший: побочная R → S "
        "замедлена, в РИВ ΠR = 50,84 кмоль/ч против 30,46 и 40,00 в случаях 1 и 2.\n\n"
        "Вывод: процесс целесообразнее вести в РИВ; из трёх кинетик оптимален случай 3."
    )
    ws2.merge_cells("A12:G18")
    c = ws2["A12"]
    c.value = text
    c.alignment = Alignment(wrap_text=True, vertical="top")
    c.font = Font(name="Calibri", size=12)
    ws2.row_dimensions[12].height = 90
