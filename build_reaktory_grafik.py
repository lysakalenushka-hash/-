...
def build():
    ...
    text = (
        "1. Для всех трёх кинетических случаев РИВ даёт большую степень превращения X и большую "
        "производительность ΠR по целевому продукту R, чем РИС-н того же объёма (τ = 2 мин). "
        "Это ожидаемо: в РИС-н вся реакция идёт при низкой выходной CA, скорость A→R меньше.\n\n"
        "2. Селективность. В случаях 1 и 2 (n2 = 1) дифференциальная S на выходе РИС-н одинакова "
        "(0,5556), потому что отношение скоростей на выходе определяется k1, k2 и порядками. "
        "Интегральная SR = Y/X совпадает с S для РИС-н (идеальное смешение). В РИВ SR выше, чем в РИС-н: "
        "случай 1 — 0,620 против 0,556; случай 2 — 0,636 против 0,556; случай 3 — 0,808 против 0,753.\n\n"
        "3. Случай 3 (n2 = 2) наиболее выгоден по целевому продукту: побочная R→S второго порядка, "
        "CS растёт медленнее, Y и ΠR максимальны (РИВ: CR = 16,95 моль/л, ΠR = 50,84 кмоль/ч).\n\n"
        "4. Случай 1 (n1 = 2) наименее выгоден: вторая реакция по A тормозит превращение, X и ΠR минимальны.\n\n"
        "5. Для графиков выбран случай 1 (как на скриншотах РИВ с полной сеткой τ). Точки на графике — "
        "квадраты (CA), звёзды (CR) и кресты-звёздочки (CS), линии сглаженные, как в Kniga1.xlsx."
    )
    ws2.merge_cells("A12:G22")
    c = ws2["A12"]
    c.value = text
    c.alignment = Alignment(wrap_text=True, vertical="top")
    c.font = Font(name="Calibri", size=12)
    ws2.row_dimensions[12].height = 40
