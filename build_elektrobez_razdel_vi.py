#!/usr/bin/env python3
"""Раздел VI Приложения 2 — субъекты оперативно-диспетчерского управления (актуализация 2026)."""
...
OUT_DIR = Path("/workspace/elektrobezopasnost_razdel_VI")
OUT = OUT_DIR / "Elektrobezopasnost_razdel_VI.xlsx"
OUT_CYR = OUT_DIR / "Электробезопасность_раздел_VI.xlsx"
OUT_LMS = OUT_DIR / "Elektrobezopasnost_razdel_VI_LMS.xlsx"
OUT_LMS_CYR = OUT_DIR / "Электробезопасность_раздел_VI_тест.xlsx"

FOLDER = "Электробезопасность. Раздел VI"
PROGRAM = "Электробезопасность. Раздел VI"
...
def parse_section_vi(path: Path):
...
def build(topics, questions):
    ...
    w0.title = "0. Сводка"
    w0["A1"] = (
        "Приложение 2. Раздел VI — работники субъектов оперативно-диспетчерского управления"
    )
    w0["A1"].font = TITLE
    ...
    w0["A2"] = (
        ...
        "В исходном xlsx шапка листа VI ошибочно подписана как теплоснабжающие организации; "
        "по составу тем это раздел VI официального перечня (ОДУ / системный оператор): 452 вопроса, 9 НПА. "
        "Сверка с КонсультантПлюс LAW_541956: приказ 548 — 20/20; ПП 937 — 72/72; приказ 757 — 29/29 из 50 (фрагмент); "
        "ПТЭЭСС — 104/104 из 214 (фрагмент); пожарная безопасность, персонал и первая помощь совпадают с проверенным разделом II. "
        "Содержательных расхождений нет. Правильный ответ в компоновке Приложения 2 — первый вариант."
    )
    w0["A2"].alignment = Alignment(wrap_text=True, vertical="center")
    w0.merge_cells("A2:F2")
    w0.row_dimensions[2].height = 96
    ...
    w1["A1"] = "Раздел VI. Вопросы и варианты ответов (верный — вариант 1)"
    w1["A1"].font = TITLE
    ...
    src = cmp["Сравнение"]
    w3["A1"] = "Раздел VI. Сопоставление со старым банком (файл Voprosi-po-elektrobezopasnosti-2026)"
    w3["A1"].font = TITLE
    ...
    w0["A20"] = "Изменения относительно предыдущего банка (раздел VI)"
    ...
    w4["A1"] = "Проверка актуальности раздела VI относительно официального перечня РТН"
    w4["A1"].font = TITLE
    w4.merge_cells("A1:B1")
    w4["A2"] = (
        "Источник: перечень утв. 10.08.2026, применяется с 01.09.2026 "
        "(КонсультантПлюс LAW_541956). "
        "Сверка 23.09.2026: 452 вопроса раздела VI (ОДУ). Шапка листа VI в исходном xlsx подписана неверно. "
        "Содержательных расхождений с официальным текстом нет."
    )
    ...
    rows = [
        ("Раздел официального перечня", "VI. Работники субъектов оперативно-диспетчерского управления"),
        ("Вопросов в Приложении 2 (лист VI)", len(questions)),
        ("Вопросов в официальном перечне", 452),
        ("Совпадение формулировок", "452 / 452"),
        ("Тем (НПА)", len(topics)),
        ("Старый банк (до 01.09.2026)", "455 вопросов"),
...
if __name__ == "__main__":
    topics, questions = parse_section_vi(SRC_APP)
