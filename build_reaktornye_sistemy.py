...
def build():
    ...
    r = write_scheme(
        ...
        "(vi = 50 л/мин, τi = 0,5 мин). Вход и выходы реакторов 1–4, экран программы.",
    )
    r += 1
    r = write_block(w2, r, "Система 1", rows_cstr_ser) + 2
    ...
    r += 1
    r = write_block(w2, r, "Система 2", rows_cstr_par) + 2

    r = write_scheme(
        w2,
        r,
        "Таблица 3. Система 3 — четыре РИВ последовательно "
        "(vi = 50 л/мин, Στ = 2 мин). Эквивалентно одному РИВ.",
    )
    r += 1
    r = write_block(w2, r, "Система 3", rows_pfr_par) + 2

    r = write_scheme(
        w2,
        r,
        "Таблица 4. Система 4 — четыре РИВ параллельно "
        "(vi = 12,5 л/мин, τi = 2 мин). Выход как у единичного РИВ.",
    )
    r += 1
    r = write_block(w2, r, "Система 4", rows_pfr_par) + 2
