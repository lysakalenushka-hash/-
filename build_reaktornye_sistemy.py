...
def build():
    ...
    # Как на экране: τ = 0 и τ = 0,5. Выход — системы целиком.
    rows_sys = [
        [(0.0, 0.0000, 0.0000, 0.0000), (0.5, 0.6498, 0.3964, 0.2535)],
        [(0.0, 0.0000, 0.0000, 0.0000), (0.5, 0.5454, 0.3030, 0.2424)],
        [(0.0, 0.0000, 0.0000, 0.0000), (0.5, 0.6988, 0.4444, 0.2544)],
        [(0.0, 0.0000, 0.0000, 0.0000), (0.5, 0.6988, 0.4444, 0.2544)],
    ]
    ...
    w2["A1"] = "Пункт 2. Четыре таблицы — по одной на каждую исследуемую систему"
    w2["A1"].font = TITLE_FONT
    w2.merge_cells("A1:G1")
    r = 3

    titles = [
        "Таблица 1. Система 1 — 4 РИС-н последовательно (vi = 50 л/мин, τi = 0,5 мин). Экран: τ = 0 и 0,5.",
        "Таблица 2. Система 2 — 4 РИС-н параллельно. Экран: τ = 0 и 0,5.",
        "Таблица 3. Система 3 — 4 РИВ последовательно (τi = 0,5 мин). Экран: τ = 0 и 0,5.",
        "Таблица 4. Система 4 — 4 РИВ параллельно. Экран: τ = 0 и 0,5.",
    ]
    for i, (title, rows) in enumerate(zip(titles, rows_sys), 1):
        r = write_scheme(w2, r, title)
        r += 1
        r = write_block(w2, r, f"Система {i}", rows) + 2
