...
def build():
    ...
    pfr_series = []
    for _i in range(4):
        ca0, cr0, cs0 = ca_in, cr_in, cs_in
        x0, y0, z0, _ = xyz_from_c(ca0, cr0, cs0)
        ca_in, cr_in, cs_in = pfr_step(0.5, ca0, cr0, cs0)
        x, y, z, _ = xyz_from_c(ca_in, cr_in, cs_in)
        pfr_series.append(
            [
                (0.0, round(x0, 4), round(y0, 4), round(z0, 4)),
                (0.5, round(x, 4), round(y, 4), round(z, 4)),
            ]
        )
    x0, y0, z0, _ = xyz_from_c(1.0, 0.0, 0.0)
    ca_p, cr_p, cs_p = pfr_step(2.0, 1.0, 0.0, 0.0)
    x, y, z, _ = xyz_from_c(ca_p, cr_p, cs_p)
    rows_pfr_par = [
        (0.0, round(x0, 4), round(y0, 4), round(z0, 4)),
        (2.0, round(x, 4), round(y, 4), round(z, 4)),
    ]
    rows_cstr_par = [
        (0.0, 0.0000, 0.0000, 0.0000),
    ...
    w2["A1"] = (
        "Пункт 2. Таблица с результатами моделирования для каждого реактора каждой системы "
        "(колонки как в бланке: Тау, X, CA, Y, CR, Z, CS)"
    )
    ...
    r = write_scheme(
        ...
        "(vi = 50 л/мин, τi = 0,5 мин). Экран программы, локальное Тау аппарата.",
    )
    r += 1
    for i in range(1, 5):
    ...
    r = write_scheme(
        w2,
        r,
        "Система 2. Четыре параллельно соединённых РИС-н, одинаковая нагрузка "
        "(vi = 12,5 л/мин, τi = 2 мин). Аппараты одинаковы.",
    )
    r += 1
    for i in range(1, 5):
    ...
    r = write_scheme(
        w2,
        r,
        "Система 4. Четыре параллельно соединённых РИВ, одинаковая нагрузка "
        "(vi = 12,5 л/мин, τi = 2 мин). Аппараты одинаковы.",
    )
    r += 1
    for i in range(1, 5):
