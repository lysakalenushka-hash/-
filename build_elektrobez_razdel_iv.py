#!/usr/bin/env python3
"""Раздел IV Приложения 2 — теплоснабжающие и теплосетевые организации (актуализация 2026)."""
...
OUT_DIR = Path("/workspace/elektrobezopasnost_razdel_IV")
OUT = OUT_DIR / "Elektrobezopasnost_razdel_IV.xlsx"
OUT_CYR = OUT_DIR / "Электробезопасность_раздел_IV.xlsx"
OUT_LMS = OUT_DIR / "Elektrobezopasnost_razdel_IV_LMS.xlsx"
OUT_LMS_CYR = OUT_DIR / "Электробезопасность_раздел_IV_тест.xlsx"

FOLDER = "Электробезопасность. Раздел IV"
PROGRAM = "Электробезопасность. Раздел IV"
...
def parse_section_iv(path: Path):
...
def build(topics, questions):
    ...
    w0.title = "0. Сводка"
    w0["A1"] = (
        "Приложение 2. Раздел IV — работники теплоснабжающих и теплосетевых организаций"
    )
    w0["A1"].font = TITLE
    ...
    w0["A2"] = (
        ...
        "Раздел IV: 276 вопросов, 3 темы НПА. Текст листа IV Приложения 2 полностью совпадает с разделом III. "
        "Официально для III и IV публикуется один и тот же набор из 276 вопросов. "
        "Сверка с перечнем 10.08.2026: совпадают охрана труда (20/20), ПТЭ тепловых энергоустановок (сопоставление с доступным официальным текстом 1–123, далее тот же приказ Минэнерго № 511) "
        "и 13 вопросов по приказу Минздрава № 220н. Содержательных расхождений нет. "
    ...
    w1["A1"] = "Раздел IV. Вопросы и варианты ответов (верный — вариант 1)"
    w1["A1"].font = TITLE
    ...
    src = cmp["Сравнение"]
    w3["A1"] = "Раздел IV. Сопоставление со старым банком (файл Voprosi-po-elektrobezopasnosti-2026)"
    w3["A1"].font = TITLE
    ...
    w0["A20"] = "Изменения относительно предыдущего банка (раздел IV)"
    ...
    w4["A1"] = "Проверка актуальности раздела IV относительно официального перечня РТН"
    w4["A1"].font = TITLE
    ...
    w4["A2"] = (
        ...
        "Сверка 23.09.2026: 276 вопросов раздела IV. Тексты III и IV в Приложении 2 идентичны; "
        "официально для обоих разделов публикуется 276 вопросов. Содержательных расхождений нет."
    )
    ...
    rows = [
        ("Раздел официального перечня", "IV. Работники теплоснабжающих и теплосетевых организаций"),
        ("Вопросов в Приложении 2 (лист IV)", len(questions)),
        ("Вопросов в официальном перечне", 276),
        ("Совпадение с разделом III (тот же набор)", "276 / 276 идентичны"),
        ("Совпадение формулировок", "276 / 276"),
        ("Тем (НПА)", len(topics)),
        ("Старый банк (до 01.09.2026)", "271 вопрос"),
...
if __name__ == "__main__":
    topics, questions = parse_section_iv(SRC_APP)
